...
def download_and_map_source_data(project_id, ticket_number):
    ...
    # CSV 파일 경로를 상대경로로 설정
    csv_file_path = script_dir / "bquxjobRaw.csv"
    ...
    gcs_prefix = f"1069/{project_id}_source/_source_data/"
    blobs = list(bucket.list_blobs(prefix=gcs_prefix))
    print(f"GCS 경로: {gcs_prefix}")
    ...
    if missing_columns:
        print(f"경고: 다음 컬럼이 CSV에 없습니다: {missing_columns}")
    
    # 결과 CSV 저장 (지정된 컬럼 순서로)
    output_file = script_dir / f"{ticket_number}_{project_id}.csv"
    df.to_csv(str(output_file), index=False, encoding='utf-8-sig', columns=existing_columns)
